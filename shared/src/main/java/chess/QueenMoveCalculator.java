...
public class QueenMoveCalculator extends PieceMoveCalculator {
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition position, ChessGame.TeamColor pieceColor){
        boolean[] continueSearch = {true, true, true, true, true, true, true, true}; // one element for each direction of search
        ...
        for (int i = 1; i < 8; i++){
            ...
            endPositions.add(new ChessPosition(position.getRow()+i, position.getColumn()));
            endPositions.add(new ChessPosition(position.getRow()-i, position.getColumn()));
            endPositions.add(new ChessPosition(position.getRow(), position.getColumn()+i));
            endPositions.add(new ChessPosition(position.getRow(), position.getColumn()-i));
